...
class Dialate_odd_down(nn.Module):
    def __init__(self,in_channels,hid_channels):
        super(Dialate_odd_down,self).__init__()
        # mid_channels = in_channels//2
        # self.conv1x1 = nn.Sequential(
        #     nn.Conv2d(in_channels,mid_channels,kernel_size=1,stride=1),
        #     # nn.BatchNorm2d(mid_channels, momentum=0.01, eps=1e-3),
        ...
        #     nn.ReLU(),
        # )
        self.feat_odd_block = Dilate_odd_block(in_channels, hid_channels)
        self.bn = nn.BatchNorm2d(3*hid_channels, momentum=0.01, eps=1e-3)
        self.down_channel =nn.Sequential(
            Conv2dStaticSamePadding(3*hid_channels,hid_channels, 1),
            nn.BatchNorm2d(hid_channels, momentum=0.01, eps=1e-3),
...
class D_FPN(nn.Module):
    ...
    # def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False,attention=True,use_p8=False):
        ...
        # odd-diate
        # self.feat1_odd_block = Dialate_odd_down(num_channels, num_channels)
        # self.feat2_odd_block = Dialate_odd_down(num_channels, num_channels)
        # self.feat3_odd_block = Dialate_odd_down(num_channels, num_channels)
        # self.feat4_odd_block = Dialate_odd_down(num_channels, num_channels)
        # self.feat5_odd_block = Dialate_odd_down(num_channels, num_channels)
...
# odd
#         self.p1d_dia = Dialate_odd_down(num_channels, num_channels)
#         self.p2d_dia = Dialate_odd_down(num_channels, num_channels)
#         self.p3d_dia = Dialate_odd_down(num_channels, num_channels)
#         self.p4d_dia = Dialate_odd_down(num_channels, num_channels)
#         self.p5d_dia = Dialate_odd_down(num_channels, num_channels)
    ...
    def _forward_fast_attention(self, inputs):
        ...
        #  dialte_conv--
            p1d_in_dia = self.pf1_conv(p1f)
            p2d_in_dia = self.pf2_conv(p2f)
            p3d_in_dia = self.pf3_conv(p3f)
            p4d_in_dia = self.pf4_conv(p4f)
