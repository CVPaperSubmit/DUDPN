...
class Dialate_even_down(nn.Module):
    def __init__(self,in_channels,hid_channels):
        ...
        self.down_channel =nn.Sequential(
            Conv2dStaticSamePadding(hid_channels,mid_channels, 1),
            nn.BatchNorm2d(mid_channels, momentum=0.01, eps=1e-3),
            nn.ReLU(),
            # Conv2dStaticSamePadding(mid_channels, hid_channels, 1),
                                         )
    def forward(self,x):
        x=F.relu(x)
        # x=self.conv1x1(x)
...
class D_FPN(nn.Module):
    ...
    # def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4, onnx_export=False,attention=True,use_p8=False):
        ...
        # self.use_p8 = use_p8
        # even-diate
        self.feat1_even_block = Dilate_even_block(num_channels, num_channels)
        self.feat2_even_block = Dilate_even_block(num_channels, num_channels)
        self.feat3_even_block = Dilate_even_block(num_channels, num_channels)
        self.feat4_even_block =Dilate_even_block(num_channels, num_channels)
        self.feat5_even_block = Dilate_even_block(num_channels, num_channels)
