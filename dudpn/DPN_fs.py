...
class Dilate_odd_block(nn.Module):
    def __init__(self,num_channels,hid_channels):
        ...
        self.conv_r1 = nn.Sequential(
            nn.Conv2d(num_channels, hid_channels, kernel_size=3, stride=1, padding=1, dilation=1),
...
class D_FPN(nn.Module):
    ...
    def _forward_fast_attention(self, inputs):
        ...
        #  dialte_conv--
            p1d_in_dia = self.feat1_odd_block(p1f)
            p2d_in_dia = self.feat2_odd_block(p2f)
            p3d_in_dia = self.feat3_odd_block(p3f)
            p4d_in_dia = p4f
