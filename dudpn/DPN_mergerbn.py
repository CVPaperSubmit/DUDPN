...
class D_FPN(nn.Module):
    ...
    def _forward_fast_attention(self, inputs):
        ...
        #  第一次横向连接_____
            ...
            p1d_in  = self.p1d_dia(p1f)
            p2d_in = self.p1d_dia(p2f)
            p3d_in = self.p1d_dia(p3f)
            p4d_in = p4f
        ...
        #  dialte_conv--
            ...
            p1d = self.conv1_down(self.swish(weight[0]*p1d_in+weight[1]*p1))

            p2_down_w2 =self.p2_down_w2_relu(self.p2_down_w2)
            weight = p2_down_w2/(torch.sum(p2_down_w2,dim=0)+self.epsilon)
            p2d = self.conv2_down(self.swish(weight[0]*p2d_in+weight[1]*p2+weight[2]*self.p1d_down(p1d)))

            p3_down_w2 =self.p3_down_w2_relu(self.p3_down_w2)
            weight = p3_down_w2/(torch.sum(p3_down_w2,dim=0)+self.epsilon)
            p3d = self.conv3_down(self.swish(weight[0]*p3d_in+weight[1]*p3+weight[2]*self.p2d_down(p2d)))

            p4_down_w2 =self.p4_down_w2_relu(self.p4_down_w2)
            weight = p4_down_w2/(torch.sum(p4_down_w2,dim=0)+self.epsilon)
            p4d = self.conv4_down(self.swish(weight[0]*p4d_in+weight[1]*p4+weight[2]*self.p3d_down(p3d)))
            ...
            p2d_in = self.pf2_conv(p2f)
            p3d_in = self.pf3_conv(p3f)
            p4d_in = self.pf4_conv(p4f)
    ...
    def _forward(self, inputs):
        ...
        if self.first_time:
            ...
            p1d = self.conv1_down(self.swish(p1d_in + p1))

            p2d = self.conv2_down(self.swish(p2d_in + p2 + self.p1d_down(p1d)))

            p3d = self.conv3_down(self.swish(p3d_in + p3 + self.p2d_down(p2d)))

            p4d = self.conv4_down(self.swish(p4d_in + p4 + self.p3d_down(p3d)))
